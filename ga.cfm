...
<table style="font-family: Gotham, 'Helvetica Neue', Helvetica, Arial, 'sans-serif'" cellspacing="1" cellpadding="4" bgcolor="grey">
	<cfloop query="qryRecord">
		...
		<cftry>
			...
			<cfhttp method="POST" url="https://ltxx.bunity.com/v1/ga" result="resultText">
				...
				<cfhttpparam name="company" type="formfield" value="#trim(qryRecord.entityName)#">
			</cfhttp>

			<cfset json = DeserializeJSON(resultText.Filecontent)>
			
			<cfif isArray(json) AND ArrayLen(json) GT 0>
				<cfset json = json[1]>
			</cfif>
			...
			<cfelse>
				...
				<tr bgcolor="white">
					<td>#json.name#</td>
					<td>License type: <cftry>#json.license_type#<cfcatch>err</cfcatch></cftry></td>
					<td>License: #json.license# Exp: #dateformat(left(json.expiration_date,10),'mm/dd/yyyy')#</td>
					<td>Firm: 
                        <cftry>#json.firm_name#<cfcatch>err</cfcatch></cftry>  (Role: <cftry>#json.firm_role#<cfcatch>err</cfcatch></cftry>)
                    </td>
