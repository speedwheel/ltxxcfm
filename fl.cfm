...
<cfif previewSearch neq "">
	<cfquery name="getRecord">
		...
		WHERE lc.fk_stateID = 10 <!---this is the state ID--->
...
<table style="font-family: Gotham, 'Helvetica Neue', Helvetica, Arial, 'sans-serif'" cellspacing="1" cellpadding="4" bgcolor="grey">
	<cfloop query="qryRecord">
		...
		<cftry>
			...
			<cfhttp method="POST" url="https://ltxx.bunity.com/v1/fl" result="resultText">
				<cfhttpparam type="header" name="Authorization" value="Basic a2F6ZWxpOj5zWzVaN11bRU4rayk7U2s=">
				<cfif Len(Trim(qryRecord.LicenseEmail)) GT 1 AND Len(Trim(qryRecord.LicensePassword))>
					<cfhttpparam name="username" type="formfield" value="#qryRecord.LicenseEmail#">
					...
					<table>
						...
						<cfloop from="1" to="#ArrayLen(json.courses)#" index="i">
							...
							<tr  bgcolor="white">
								...
								<td>#item.required#</td>
								<td>#yesNoFormat(item.completed)#</td>
								<td>#item.short_fall#</td>
								<td>#dateformat(left(item.start_date,10),'mm/dd/yyyy')#</td>
								<td>#dateformat(left(item.end_date,10),'mm/dd/yyyy')#</td>
							</tr>
						</cfloop>
					</table>
					</td>
				...
				<cfif previewSearch neq "">
					<cfquery name="addData">
						UPDATE kirks_trackerSearch SET
							...
							<cfif structKeyExists(json, "courses") and isArray(json.courses) and arrayLen(json.courses) gt 0>
								scrp_topics = <cfqueryparam value="#SerializeJSON(json.courses)#" cfsqltype="cf_sql_varchar">,
							</cfif>
