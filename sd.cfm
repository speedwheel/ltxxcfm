...
<h2>South Dakota (SD) Test Scraper</h2>
...
<cfif previewSearch neq "">
	<cfquery name="getRecord">
		...
		WHERE lc.fk_stateID = 42 <!---this is the state ID--->
...
<!--- <cfdump var="#qryRecord#" abort="true"> --->
...
<table style="font-family: Gotham, 'Helvetica Neue', Helvetica, Arial, 'sans-serif'" cellspacing="1" cellpadding="4" bgcolor="grey">
	<cfloop query="qryRecord">
		...
		<cftry>
			<cfset fullNameForAPI = "">
			<cfif Len(qryRecord.lastname)>
				<cfset fullNameForAPI = qryRecord.lastname>
			</cfif>

			
			<cfif Len(qryRecord.firstname) and Len(fullNameForAPI)>
				<cfset fullNameForAPI &= ", " & qryRecord.firstname>
			<cfelseif Len(qryRecord.firstname)>
				<cfset fullNameForAPI &= qryRecord.firstname>
			</cfif>

			<cfif Len(qryRecord.middlename) and Len(fullNameForAPI)>
				<cfset fullNameForAPI &= " " & LEFT(qryRecord.middlename,1)>
			</cfif>

			<!---These parameters/url will be defined by Edward for each state so these will vary--->
			<cfhttp method="POST" url="https://ltxx.bunity.com/v1/sd" result="resultText">
				<cfhttpparam type="header" name="Authorization" value="Basic a2F6ZWxpOj5zWzVaN11bRU4rayk7U2s=">

				<cfif qryRecord.firstname EQ "Test">
					<cfhttpparam name="name" type="formfield" value="collins, keith p">
				<cfelse>
					<cfhttpparam name="name" type="formfield" value="#fullNameForAPI#">
				</cfif>
			</cfhttp>
			...
			<cfelse>
				...
				<tr bgcolor="white">
					...
					<td>#json.license# Issued Date: #dateformat(left(json.issued_date,10),'mm/dd/yyyy')#</td>
					<td>Expiry Year: #json.renewal_year#</td>
					<td>&nbsp;</td>
				</tr>
				...
				<cfif previewSearch neq "">
					<cfquery name="addData">
						UPDATE kirks_trackerSearch SET
							...
							scrp_expire = #createODBCdate(createDate(json.renewal_year,"01","01"))#
						WHERE pk_trackerSearchID = <cfqueryparam value="#qryRecord.pk_trackerSearchID#" cfsqltype="cf_sql_integer">
					</cfquery>
				<cfelse>
					<cfquery name="addData">
