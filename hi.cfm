...
<cfif previewSearch neq "">
	<cfquery name="getRecord">
		...
		FROM kirks_trackerSearch t with (NOLOCK)

		INNER JOIN kirks_licenseScrape lc with (NOLOCK) ON lc.pk_licenseid = t.fk_licenseScrapeid

		WHERE lc.fk_stateID = 12 <!---this is the state ID--->
...
<cfelse>
	<cfquery name="getRecord">
		...
		FROM kirks_tracker t with (NOLOCK)
		INNER JOIN mb_entity en with (NOLOCK) ON en.pk_entityid = t.fk_entityid
		INNER JOIN kirks_licenseScrape lc with (NOLOCK) ON lc.pk_licenseid = t.fk_licenseScrapeid
...
<table style="font-family: Gotham, 'Helvetica Neue', Helvetica, Arial, 'sans-serif'" cellspacing="1" cellpadding="4" bgcolor="grey">
	<cfloop query="qryRecord">
		...
		<cftry>
			...
			<cfhttp method="POST" url="https://ltxx.bunity.com/v1/hi" result="resultText">
			...
			<cfelse>
				...
				<tr bgcolor="white">
					<td>
						<cftry>#json.name#<cfcatch></cfcatch></cftry>
						<cftry>#json.company#<cfcatch></cfcatch></cftry>
					</td>
					<td><!--- #json.license_type# ---></td>
					<td>#json.license# Exp: #dateformat(left(json.expiration_date,10),'mm/dd/yyyy')#</td>
					<td><!--- #json.mailing_address# ---></td>
					<td colspan="2">status: #json.status#</td>
				</tr>
