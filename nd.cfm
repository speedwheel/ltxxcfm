...
<h2>North Dakota (ND) Test Scraper</h2>
...
<cfif previewSearch neq "">
	<cfquery name="getRecord">
		...
		WHERE lc.fk_stateID = 35 <!---this is the state ID--->
...
<table style="font-family: Gotham, 'Helvetica Neue', Helvetica, Arial, 'sans-serif'" cellspacing="1" cellpadding="4" bgcolor="grey">
	<cfloop query="qryRecord">
		...
		<cftry>
			<cfif qryRecord.firstname EQ "" AND qryRecord.lastname EQ "">
				<cfthrow message="Invalid license information. firstname or lastname can't be empty">
			</cfif>

			<cfif qryRecord.firstname EQ "DevUser">
				<cfset fName = "ronn">
				<cfset lName = "thomas">
			<cfelseif qryRecord.firstname NEQ "" AND qryRecord.lastname NEQ "">
				<cfset fName = qryRecord.firstname>
				<cfset lName = qryRecord.lastname>
			</cfif>

			<!---These parameters/url will be defined by Edward for each state so these will vary--->
			<cfhttp method="POST" url="https://ltxx.bunity.com/v1/nd" result="resultText">
