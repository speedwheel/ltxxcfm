...
<h2>Nebraska (NE) Test Scraper</h2>
...
<cfif previewSearch neq "">
	<cfquery name="getRecord">
		...
		WHERE lc.fk_stateID = 28 <!---this is the state ID--->
	...
	<cfloop query="qryRecord">
		...
		<cftry>
			...
			<cfif Len(qryRecord.middlename) and Len(fullNameForAPI)>
				<cfset fullNameForAPI &= ", " & qryRecord.middlename>
			<cfelseif Len(qryRecord.middlename)>
				<cfset fullNameForAPI &= qryRecord.middlename>
			</cfif>

			<!---These parameters/url will be defined by Edward for each state so these will vary--->
			<cfhttp method="POST" url="https://ltxx.bunity.com/v1/ne" result="resultText">
				<cfhttpparam type="header" name="Authorization" value="Basic a2F6ZWxpOj5zWzVaN11bRU4rayk7U2s=">
				<cfhttpparam name="license_type" type="formfield" value="#qryRecord.l_dispname#">

				<cfif qryRecord.firstname EQ "Test">
					<cfhttpparam name="name" type="formfield" value="Dean, Robert, Allan">
				<cfelse>
