...
<h2>Colorado (CO) Test Scraper</h2>
...
<cfif previewSearch neq "">
	<cfquery name="qryRecord">
		...
		WHERE lc.fk_stateID = 6 <!---this is the state ID--->
...
<table style="font-family: Gotham, 'Helvetica Neue', Helvetica, Arial, 'sans-serif'" cellspacing="1" cellpadding="4" bgcolor="grey">
	<cfloop query="qryRecord">
		...
		<cftry>
			...
			<cfhttp method="POST" url="https://ltxx.bunity.com/v1/co" result="resultText">
			...
			<cfelse>
				<tr bgcolor="white">
					<td>#json.name#</td>
					<td><cftry>#json.address#<cfcatch>..</cfcatch></cftry></td>
					<td>#json.license# Exp: <cftry>#dateformat(left(json.expiration_date,10),'mm/dd/yyyy')#<cfcatch>..</cfcatch></cftry></td>
					<td>Status: #json.status#</td>
					<td>
						Issued Date: #dateformat(left(json.issued_date,10),'mm/dd/yyyy')#
					</td>
				</tr>
				<cftry><cfsilent>#dateformat(left(json.expiration_date,10),'mm/dd/yyyy')#</cfsilent><cfcatch><cfset error = cfcatch.message></cfcatch></cftry>
