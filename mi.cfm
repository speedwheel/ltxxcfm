...
<h2>Michigan (MI) Test Scraper</h2>
...
<cfif previewSearch neq "">
	<cfquery name="qryRecord">
		...
		WHERE lc.fk_stateID = 23 <!---this is the state ID--->
...
<table style="font-family: Gotham, 'Helvetica Neue', Helvetica, Arial, 'sans-serif'" cellspacing="1" cellpadding="4" bgcolor="grey">
	<cfloop query="qryRecord">
		...
		<cftry>
			...
			<cfhttp method="POST" url="https://ltxx.bunity.com/v1/mi" result="resultText">
						...
						<td colspan="5">
							<table>
								...
								<cfloop from="1" to="#ArrayLen(json.courses)#" index="i">
									...
									<tr>
										<td><cftry>#item.course#<cfcatch>..</cfcatch></cftry></td>
										<td>
											<cfif not IsNull(item.education_provider)>
												#item.education_provider#
											</cfif>
										</td>
										<td><cftry>#item.hours#<cfcatch>..</cfcatch></cftry></td>
										<td><cftry>#item.course_id#<cfcatch>..</cfcatch></cftry></td>
...
<cffunction name="getCoursesTotal" returntype="struct" hint="Gets the total hours of Core and elective courses" >
	...
	<cfscript>
		...
		for( var course in arguments.courses ){
			try { 
				if(course.series EQ "F"){
					loc.Req += course.hours;
					totalUnits += course.hours;
				}
				else{
					loc.Elec += course.hours;
					totalUnits += course.hours;
				}
			} 
			catch (any e) {}
